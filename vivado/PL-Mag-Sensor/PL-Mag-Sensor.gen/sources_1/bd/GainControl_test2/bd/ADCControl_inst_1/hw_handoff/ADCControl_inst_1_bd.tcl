...
################################################################
# This is a generated script based on design: ADCControl_inst_1
#
...
# source ADCControl_inst_1_script.tcl


# The design that will be created by this Tcl script contains the following 
# module references:
# ADC_controller, max_gain, counter
...
set design_name ADCControl_inst_1
...
proc create_root_design { parentCell } {
  ...
  # Create instance: max_gain_0, and set properties
  set block_name max_gain
  set block_cell_name max_gain_0
  if { [catch {set max_gain_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $max_gain_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  connect_bd_net -net SampleTimeLUT_douta [get_bd_pins SampleTimeLUT/douta] [get_bd_pins sample_time_counter/target]
  ...
  connect_bd_net -net curr_gain_1 [get_bd_ports curr_gain] [get_bd_pins max_gain_0/gains]
  connect_bd_net -net gpio_UnD_ref_1 [get_bd_ports gpio_UnD_ref] [get_bd_pins ADC_controller/gpio_UnD_ref]
  connect_bd_net -net gpio_nCS_ref_1 [get_bd_ports gpio_nCS_ref] [get_bd_pins ADC_controller/gpio_nCS_ref]
  connect_bd_net -net max_gain_0_max_gain_out [get_bd_pins SampleTimeLUT/addra] [get_bd_pins max_gain_0/max_gain_out]
  ...
  connect_bd_net -net sample_time_counter_irq [get_bd_pins ADC_controller/t_sample_irq] [get_bd_pins sample_time_counter/irq]
  connect_bd_net -net spi_din_1 [get_bd_ports spi_din] [get_bd_pins ADC_controller/spi_din]
  connect_bd_net -net spi_irq_1 [get_bd_ports spi_irq] [get_bd_pins ADC_controller/spi_irq]
